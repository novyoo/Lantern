...
def apply_local_status(new_status, current_status, identity):
    ...
    if new_status == "ACTIVE":
        ...
        certificate = vault.build_and_sign_certificate(
            device_id, identity["rental_id"], socket.gethostname(), reason="Rental erasure confirmed"
        )
        print("Erasure certificate signed with this device's own key - sending it to the server next check-in.")
    return new_status, certificate

def checkin(identity, current_status, certificate):
    body = {
        "device_id": identity["device_id"],
    ...
    if response.status_code == 401:
        ...
        return current_status, certificate
    response.raise_for_status()
    data = response.json()
    new_status, new_certificate = apply_local_status(data["device_status"], current_status, identity)
    return new_status, certificate or new_certificate
...
def main():
    ...
    while True:
        try:
            current_status, certificate = checkin(identity, current_status, certificate)
